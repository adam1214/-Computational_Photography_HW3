...
def get_kernel(gau_N=13, gau_std=1.2, mv_x=0, mv_y=0):
    ...
    kernel_temp = np.zeros((gau_N, gau_N))  #kernel_temp = np.array(11, 11) 
    gau_N_half = int(gau_N / 2) #gau_N_half = 5
    Cx = -1.5 + 4 * mv_x
    Cy = -1.5 + 4 * mv_y
    
    for kernel_row in range(gau_N):
        for kernel_column in range(gau_N):
            k = -gau_N_half + kernel_column
            l = -gau_N_half + kernel_row
            kernel_temp[kernel_row, kernel_column] = (Cx - k) ** 2 + (Cy - l) ** 2
    
    kernel_temp = np.exp(-(kernel_temp) / (2 * (gau_std ** 2)))
    kernel = (kernel_temp) / (np.sum(kernel_temp))
    return kernel
...
def solve(imgs, filters, lamb):
    ...
    first_term = norm1(subsample(conv(filters[0], x, dims=2), (4, 4, 1)) - imgs[0])
    for i in range(len(imgs) - 1):
        first_term = first_term + norm1(subsample(conv(filters[i + 1], x, dims=2), (4, 4, 1)) - imgs[i + 1])

    prob = Problem(first_term + lamb*group_norm1(grad(x, dims=2), [3]))
...
if __name__ == '__main__':
    ...
    imageio.imwrite('./result/zebra_test_eight_chia.png', img_out)
    # check answer
    img_urs_path = './result/zebra_test_eight_chia.png'
